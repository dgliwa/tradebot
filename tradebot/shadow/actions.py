...
def apply_corporate_actions(
    ...
    rows = conn.execute(
        """SELECT a.id,a.ticker,a.action_type,a.value FROM corporate_actions a
    ...
    count = 0
    for action_id, ticker, action_type, value in rows:
        held = {item.ticker: item.quantity for item in positions(conn, account.id)}.get(ticker, 0)
